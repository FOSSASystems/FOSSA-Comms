...
// spreading factor to be used in transmission
#define ALT_SPREADING_FACTOR        12
...
void loop() {
  ...
  if(millis() - lastTransmit >= transmitPeriod) {
    ...
    // data to transmit
    uint8_t functionId = CMD_TRANSMIT_CALLSIGN_ALT_SF;
    uint8_t optData[] = {ALT_SPREADING_FACTOR};
    uint8_t optDataLen = 1;
    ...
    // set radio mode to reception with alternate SF
    Serial.println(F("Waiting for response ... "));
    radio.setSpreadingFactor(ALT_SPREADING_FACTOR);
    radio.setDio1Action(setFlag);
    radio.startReceive();
  ...
  if(receivedFlag) {
    ...
    if (state == ERR_NONE) {
      ...
      if(functionId == RESP_CALLSIGN_ALT_SF) {
        Serial.println(F("Got alternate SF callsign!"));
      }
